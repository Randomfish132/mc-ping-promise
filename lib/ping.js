...
function ping( hostname, port ) {
   ...
   if( hostType !== 'string' ) {
      return Promise.reject( 'hostname must be a string not ' + hostType );
   }
   ...
   if( portType === 'undefined' ) {
      ...
      if( isNaN( port ) ) return Promise.reject( 'port must be a number not ' + portType );
   }

   if( port < 1 || port > 65535 ) {
      return Promise.reject( 'port must be in range 1-65535' );
   }
